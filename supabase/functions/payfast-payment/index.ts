...
function generateSignature(data: Record<string, string>, passphrase: string): string {
  ...
  // Use Web Crypto API for MD5-equivalent via SubtleCrypto isn't available for MD5
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Build the notify URL (webhook)
    const notifyUrl = `${SUPABASE_URL}/functions/v1/payfast-webhook`;

    // Payfast payment data - keys must be in specific order
    const paymentData: Record<string, string> = {
      ...
      name_first: "",
      email_address: claimsData.claims.email || "",
    ...
    // Sandbox merchant IDs are typically "10000100" for testing
